...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* =========================================================================
       COMPACT THEME SELECTOR — color-dot radio buttons in minimal rows
       ========================================================================= */
    .theme-selector-bar {
      display: flex;
      flex-wrap: wrap;
    ...
    .theme-dot-btn:hover { background: var(--theme-header-bg); }
    ...
    .theme-dot-round { border-radius: 50%; }
    .theme-sep {
      width: 1px;
      height: 18px;
    ...
    /* Arrow navigation for circular view cycling */
    .col-nav {
      display: none;
    }
    @media (max-width: 720px) {
      ...
      .col-nav-btn:hover { background: var(--theme-header-bg); }
      .col-nav-label {
        font-size: 0.82rem;
        font-weight: 700;
        color: var(--theme-fg);
    ...
    /* 2-Column Responsive Grid Layout — replaced by columns-layout */
    .app-layout {
      display: flex;
      flex-direction: column;
      gap: 0.75rem;
    ...
    /* Left Sidebar — removed, replaced by theme-selector-bar */
    .theme-chip {
      display: inline-flex;
      align-items: center;
      gap: 0.35rem;
...
<body>
  ...
  <main class="viewport">
    <div class="columns-layout">
      ...
      <!-- COLUMN 1: Dashboard metrics + table -->
      ...
      <!-- COLUMN 2: Chart + Signup Form -->
      <div class="col-panel" id="col-2">
        <div class="col-title">📈 Chart & Form</div>
      ...
      <!-- COLUMN 3: IDE Code Editor -->
      <div class="col-panel" id="col-3">
        <div class="col-title">💻 Code Editor</div>
  ...
  <script>
    ...
    function populateThemeBar() {
      ...
      function shortLabel(theme) {
        ...
        let base = match ? match[1] : theme.name.split(' ')[0];
        if (theme.mode === 'dark') base += ' Night';
        return base;
      }
      ...
      lightThemes.forEach((theme, i) => {
        ...
        btn.innerHTML = '<span class="theme-dot" style="background:' + theme.swatch + '"></span>' + shortLabel(theme);
    ...
    document.getElementById('col-prev').onclick = () => {
      activeColIdx = (activeColIdx - 1 + colPanels.length) % colPanels.length;
      selectCol(activeColIdx);
    };
    document.getElementById('col-next').onclick = () => {
      activeColIdx = (activeColIdx + 1) % colPanels.length;
      selectCol(activeColIdx);
    };
...
console.log('✅ Generated 4-Column Multi-Panel Interactive Theme Showcase at docs/previews/gallery.html');
