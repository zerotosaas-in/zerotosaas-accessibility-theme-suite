...
- Antigravity's Unified State Sync is a Google-account feature; disabling theme sync is a user preference, not something the extension can control. The extension can only stop _its own_ restart-time resets.
...
## Resolution implemented (2026-09-07)

- `package.json`: added `zerotosaas.theme.applyDefaultOnFirstRun` (default `false`) so the extension never writes `workbench.colorTheme` or the `preferredDark`/`preferredLight` themes unless the user explicitly opts in.
- `src/extension.js`: `applyDefaultThemeOnce` now requires the opt-in setting, uses `workbench.inspect('colorTheme')` to detect any explicit user/workspace/folder value before writing, and writes a versioned `globalState` key (with a stable fallback) so a version bump does not silently reset the gate.
- `scripts/update-ide-configs.js`: the stale-entry cleanup now removes every `*.zerotosaas-theme-*` folder and `*.zerotosaas-theme` `extensions.json` entry (old and new publisher ids) in all target IDEs, and a Devin IDE target (`~/.devin/extensions`) was added.

## Does symlinking the local dev folder share the theme setting?

No. Symlinking `vsx-theme-zerotosaas` into `~/.antigravity-ide/extensions` and `~/.windsurf/extensions` only makes the two IDEs load the same extension source. The active theme is stored in each IDE's own `User/settings.json` and each IDE's `User/globalStorage/state.vscdb`, not in the extension folder. The only way themes propagate across IDEs is an IDE-level settings sync (Antigravity Google Unified State Sync, VS Code Settings Sync, etc.) or a shared user-data directory — not the extension symlink itself.
